...
curl -OL http://ftp.rpm.org/releases/rpm-4.18.x/rpm-${RPM_VERSION}.tar.bz2
fi
...
PATH=$PREFIX/bin:$PATH CPPFLAGS="-I$PREFIX/include -I$PREFIX/include/nspr -I$PREFIX/include/nss" LDFLAGS="-L$PREFIX/lib" ./configure --target=$TARGET --prefix=$PREFIX --exec-prefix=$PREFIX --enable-sqlite --disable-rpath --without-archive
make
sudo make install
...
sudo cp $SCRIPT_ROOT/find-lang.sh $PREFIX/lib/rpm/
sudo cp $SCRIPT_ROOT/check-files $PREFIX/lib/rpm/
sudo cp $SCRIPT_ROOT/brp-remove-la-files $PREFIX/lib/rpm/

sudo sed -i '' -e 's!/usr/lib/rpm/!/Library/GSharpKit/lib/rpm/!g' $PREFIX/lib/rpm/find-provides
sudo sed -i '' -e 's!/usr/lib/rpm/!/Library/GSharpKit/lib/rpm/!g' $PREFIX/lib/rpm/find-requires
