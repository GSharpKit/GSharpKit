...
%package -n ucrt64-libtasn1
Summary:        MinGW Windows libtasn1 library
Requires:       pkgconfig

%description -n ucrt64-libtasn1
...
%build
%ucrt64_configure --disable-static --disable-gcc-warnings
%ucrt64_make
...
rm -rf %{buildroot}%{ucrt64_datadir}/info/
rm -rf %{buildroot}%{ucrt64_mandir}
# Remove .la and .def files
rm -f %{buildroot}%{ucrt64_libdir}/{*.la,*.def}
# Remove utilities
rm -f %{buildroot}%{ucrt64_bindir}/*.exe


%files -n ucrt64-libtasn1
%license COPYING COPYING.LESSERv2
%{ucrt64_bindir}/libtasn1-6.dll
%{ucrt64_includedir}/libtasn1.h
%{ucrt64_libdir}/libtasn1.dll.a
%{ucrt64_libdir}/pkgconfig/libtasn1.pc
...
- Fix FTBFS against latest ucrt-w64 (already resolved in upstream gnulib)

* Thu May 30 2013 Michael Cronenworth <mike@cchtml.com> - 3.3-2
- Rebuild for ucrt-filesystem changes
...
- Update to 2.9
- Renamed the base package to ucrt-libtasn1
