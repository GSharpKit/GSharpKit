...
%package -n ucrt64-libunistring
Summary: %{summary}

%description -n ucrt64-libunistring
This portable C library implements Unicode string types in three flavours:
(UTF-8, UTF-16, UTF-32), together with functions for character processing
(names, classifications, properties) and functions for string processing
(iteration, formatted output, width, word breaks, line breaks, normalization,
...
%install
%ucrt64_make install DESTDIR=$RPM_BUILD_ROOT INSTALL="install -p"
rm -f $RPM_BUILD_ROOT%{ucrt64_infodir}/dir

rm -f $RPM_BUILD_ROOT%{ucrt64_libdir}/*.la

# Move staged docs so not picked up by %%doc in main package
mv $RPM_BUILD_ROOT%{ucrt64_datadir}/doc/libunistring __doc

%files -n ucrt64-libunistring
%license COPYING COPYING.LIB
%doc AUTHORS NEWS README HACKING DEPENDENCIES THANKS ChangeLog
%doc __doc/*
%{ucrt64_bindir}/libunistring-2.dll
%{ucrt64_includedir}/*.h
%{ucrt64_includedir}/unistring
%{ucrt64_infodir}/libunistring.info*
%{ucrt64_libdir}/libunistring.dll.a
