...
# Win64
%package -n ucrt64-%{name1}
Summary:        MinGW Windows port of sqlite embeddable SQL database engine
Requires:       pkgconfig

%description -n ucrt64-%{name1}
SQLite is a C library that implements an SQL database engine. A large
subset of SQL92 is supported. A complete database is stored in a
single disk file. The API is designed for convenience and ease of use.
Applications that link against SQLite can enjoy the power and
...
%package -n ucrt64-%{name1}-static
Summary:        Static version of MinGW Windows port of sqlite library
Requires:       ucrt64-%{name1} = %{version}-%{release}

%description -n ucrt64-%{name1}-static
SQLite is a C library that implements an SQL database engine. A large
subset of SQL92 is supported. A complete database is stored in a
single disk file. The API is designed for convenience and ease of use.
Applications that link against SQLite can enjoy the power and
...
export UCRT64_CFLAGS="%{ucrt64_cflags} -DSQLITE_ENABLE_COLUMN_METADATA=1 -DSQLITE_DISABLE_DIRSYNC=1 -DSQLITE_ENABLE_FTS3=3 -DSQLITE_ENABLE_RTREE=1 -fno-strict-aliasing"

%ucrt64_configure %{!?with_tcl:--disable-tcl} --enable-all --enable-load-extension
%ucrt64_make
...
chmod 0644 %{buildroot}%{ucrt64_libdir}/libsqlite3.dll.a

%if %{with tcl}
install -d -m755 %{buildroot}%{ucrt64_datadir}/tcl%{tclversion}/sqlite3/
mv %{buildroot}%{_datadir}/tcl%{tclversion}/sqlite3/pkgIndex.tcl %{buildroot}%{ucrt64_datadir}/tcl%{tclversion}/sqlite3/
%endif
...
# Win64
%files -n ucrt64-%{name1}
%doc README.md VERSION
%{ucrt64_bindir}/sqlite3.exe
%{ucrt64_bindir}/libsqlite3-0.dll
%{ucrt64_libdir}/libsqlite3.dll.a
%{ucrt64_includedir}/sqlite3.h
%{ucrt64_includedir}/sqlite3ext.h
%{ucrt64_libdir}/pkgconfig/sqlite3.pc
%if %{with tcl}
%{ucrt64_datadir}/tcl%{tclversion}/sqlite3/
%{ucrt64_datadir}/tcl%{tclversion}/sqlite3/pkgIndex.tcl
%endif

%files -n ucrt64-%{name1}-static
%{ucrt64_libdir}/libsqlite3.a
...
- Update to 3.7.14.1
- Dropped all patches which are not needed for the ucrt target
...
- Renamed the source package to ucrt-sqlite (#800450)
