...
BuildRequires:  ucrt64-gnutls >= 2.10
BuildRequires:  ucrt64-gsettings-desktop-schemas
...
%package -n ucrt64-glib-networking
Summary:        MinGW Windows glib-networking library
Requires:       ucrt64-gsettings-desktop-schemas

%description -n ucrt64-glib-networking
This package contains modules that extend the networking support in GIO.
...
mkdir build_ucrt
pushd build_ucrt
%ucrt64_meson -Dlibproxy=disabled -Denvironment_proxy=enabled -Dinstalled_tests=false -Ddebug_logs=false
ninja
popd
...
rm -f %{buildroot}%{ucrt64_libdir}/gio/modules/*.dll.a
rm -f %{buildroot}%{ucrt64_libdir}/gio/modules/*.la

%files -n ucrt64-glib-networking
%license COPYING
%{ucrt64_libdir}/gio/modules/libgiognutls.dll
%{ucrt64_libdir}/gio/modules/libgioenvironmentproxy.dll
%{ucrt64_libdir}/gio/modules/libgiognomeproxy.dll
%{ucrt64_datadir}/locale/


%changelog
* Thu Jan 22 2026 Sandro Mani <manisandro@gmail.com> - 2.80.1-4
- Add Requires: ucrt32-gsettings-desktop-schemas (#2431589)
...
- Renamed the source package to ucrt-glib-networking (RHBZ #800391)
