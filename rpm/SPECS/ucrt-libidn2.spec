...
# ucrt32-gcc and ucrt64-gcc are not available on s390x builders
%if 0%{?rhel}
...
# Win64
%package -n ucrt64-libidn2
Summary:        MinGW Windows IDN 2008 library the win64 target
Requires:       pkgconfig

%description -n ucrt64-libidn2
Libidn2 is an implementation of the IDNA2008 specifications in RFC
5890, 5891, 5892, 5893 and TR46 for internationalized domain names
(IDN). It is a standalone library, without any dependency on libidn.

%package -n ucrt64-libidn2-static
Summary:        Static version of the MinGW Windows IDN 2008 library
Requires:       ucrt64-libidn2 = %{version}-%{release}

%description -n ucrt64-libidn2-static
...
%build
%ucrt64_configure --disable-nls --enable-static --enable-shared
...
rm -r $RPM_BUILD_ROOT%{ucrt64_infodir}
rm -r $RPM_BUILD_ROOT%{ucrt64_mandir}/man*

# The .def file isn't interesting for other libraries/applications
rm -f $RPM_BUILD_ROOT%{ucrt64_libdir}/libidn2-*.def

# The executables are not useful in this build
rm -f $RPM_BUILD_ROOT%{ucrt64_bindir}/lookup.exe
rm -f $RPM_BUILD_ROOT%{ucrt64_bindir}/register.exe
...
# Win64
%files -n ucrt64-libidn2
%license COPYING COPYING.LESSERv3 COPYING.unicode COPYINGv2
%{ucrt64_bindir}/idn2.exe
%{ucrt64_bindir}/libidn2-0.dll
%{ucrt64_libdir}/libidn2.dll.a
%{ucrt64_libdir}/pkgconfig/libidn2.pc
%{ucrt64_includedir}/idn2.h

%files -n ucrt64-libidn2-static
%{ucrt64_libdir}/libidn2.a
